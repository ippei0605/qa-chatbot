...
const insertDocuments = (db) => {
    const data = JSON.parse(readFile(CONTENT_FILENAME));
    db.bulk(data, (err) => {
        if (!err) {
...
// NLC の Classifier が無い場合、Classifier を作成する。
const createClassifier = () => {
    context.nlc.list({}, (err, response) => {
        if (err) {
            console.log('error:', err);
        } else {
            if (response.classifiers.length <= 0) {
                console.log('アプリに必要な NLC の Classifier がありません。');
                const params = {
                    "language": "ja",
                    "name": "classifier",
